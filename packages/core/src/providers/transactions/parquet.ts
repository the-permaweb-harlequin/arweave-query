...
import { QueryBuilder, FieldSelector, QueryValidator } from "../../utils/query-builder.js";
import { base64urlToBytes, bytesToBase64url, base64urlToBase64 } from "../../utils/encoding.js";
import type { ParquetTransactionsRow, ParquetBlocksRow, ParquetTagsRow } from "../../generated/parquet-types.js";
...
export class ParquetProvider implements QueryProvider {
  ...
  async transformParquetTransaction(row: ParquetTransactionsRow): Promise<Transaction> {
  ...
  const anchor = (anchorBytes && anchorBytes.length > 0) ? bytesToBase64url(row.anchor) : undefined;
  ...
  const transaction = {
    ...
    transaction.block = this.transformParquetBlock(blockRow[0] as ParquetBlocksRow);
    }
    return transaction;
}
  ...
  */
  ...
  async getTransaction(id: string): Promise<Transaction> {
    ...
    const transaction = await this.transformParquetTransaction(results[0] as ParquetTransactionsRow);
  ...
  async getTransactions(
    ...
    const whereConditions = this.queryBuilder.buildTransactionWhereConditions(filter);
    ...
    if (filter.tags && filter.tags.length > 0) {
      ...
      results = await this.getTransactionsWithTagFilter(filter, limit, offset, orderBy);
    } else {
    ...
    const transformedResults = await Promise.all(results.map(row => 
  ...
  private async getTransactionsWithTagFilter(
    ...
    const whereConditions = this.queryBuilder.buildTransactionWhereConditions(otherFilters);
    const orderByWithPrefix = this.queryBuilder.buildTransactionOrderBy(filter, 'tx');
    
    // Build tag filter conditions
    const tagConditions = tags!.map(tagFilter => {
      if (tagFilter.values.length === 1) {
        return this.sql`(t.tag_name = ${tagFilter.name} AND t.tag_value = ${tagFilter.values[0]})`;
      } else {
        return this.sql`(t.tag_name = ${tagFilter.name} AND t.tag_value IN (${tagFilter.values}))`;
      }
    });
  ...
  async getBlocks(filter: BlocksQueryFilter): Promise<QueryResult<Block>> {
    ...
    const transformedResults = await Promise.all(results.map(row => 
      this.transformParquetBlock(row as ParquetBlocksRow)
    ));
