...
export class QueryBuilder {
  ...
  buildTransactionWhereConditions(filter: TransactionsQueryFilter) {
    ...
    if (filter.block?.min) {
      conditions.push(
        this
          .sql`${this.sql.raw(ParquetTransactionsColumns.height)} >= ${filter.block.min}`,
      );
    }
    ...
    if (filter.block?.max) {
      conditions.push(
        this
          .sql`${this.sql.raw(ParquetTransactionsColumns.height)} <= ${filter.block.max}`,
      );
    }
    ...
    if (filter.owners && filter.owners.length > 0) {
      ...
      if (ownerBase64Strings.length === 1) {
        ...
        if (typeof base64Str === "string") {
          conditions.push(
            this
              .sql`base64(${this.sql.raw(ParquetTransactionsColumns.owner_address)}) = ${base64Str}`,
          );
        } else {
          // Fallback to binary comparison for non-string (unit test case)
          conditions.push(
            this
              .sql`${this.sql.raw(ParquetTransactionsColumns.owner_address)} = ${base64Str}`,
          );
        }
      } else {
        // For multiple owners, use IN with base64 comparison
        const stringFilters = ownerBase64Strings.filter(
          (s) => typeof s === "string",
        ) as string[];
        const blobFilters = ownerBase64Strings.filter(
          (s) => typeof s !== "string",
        );
        ...
        if (stringFilters.length > 0) {
          conditions.push(
            this
              .sql`base64(${this.sql.raw(ParquetTransactionsColumns.owner_address)}) IN (${stringFilters})`,
          );
        }
        if (blobFilters.length > 0) {
          conditions.push(
            this
              .sql`${this.sql.raw(ParquetTransactionsColumns.owner_address)} IN (${blobFilters})`,
          );
        }
      }
    }
    ...
    if (filter.recipients && filter.recipients.length > 0) {
      ...
      if (recipientBase64Strings.length === 1) {
        ...
        if (typeof base64Str === "string") {
          conditions.push(
            this
              .sql`base64(${this.sql.raw(ParquetTransactionsColumns.target)}) = ${base64Str}`,
          );
        } else {
          conditions.push(
            this
              .sql`${this.sql.raw(ParquetTransactionsColumns.target)} = ${base64Str}`,
          );
        }
      } else {
        const stringFilters = recipientBase64Strings.filter(
          (s) => typeof s === "string",
        ) as string[];
        const blobFilters = recipientBase64Strings.filter(
          (s) => typeof s !== "string",
        );
        ...
        if (stringFilters.length > 0) {
          conditions.push(
            this
              .sql`base64(${this.sql.raw(ParquetTransactionsColumns.target)}) IN (${stringFilters})`,
          );
        }
        if (blobFilters.length > 0) {
          conditions.push(
            this
              .sql`${this.sql.raw(ParquetTransactionsColumns.target)} IN (${blobFilters})`,
          );
        }
      }
    }
  ...
  buildBlockWhereConditions(filter: BlocksQueryFilter) {
    ...
    if (filter.height?.min) {
      conditions.push(
        this
          .sql`${this.sql.raw(ParquetBlocksColumns.height)} >= ${filter.height.min}`,
      );
    }
    ...
    if (filter.height?.max) {
      conditions.push(
        this
          .sql`${this.sql.raw(ParquetBlocksColumns.height)} <= ${filter.height.max}`,
      );
    }
    ...
    if (filter.ids && filter.ids.length > 0) {
      ...
      if (idBytes.length === 1) {
        conditions.push(
          this.sql`${this.sql.raw(ParquetBlocksColumns.hash)} = ${idBytes[0]}`,
        );
      } else {
        conditions.push(
          this.sql`${this.sql.raw(ParquetBlocksColumns.hash)} IN (${idBytes})`,
        );
      }
    }
  ...
  buildTransactionOrderBy(
    ...
    const heightCol = tablePrefix
      ? `${tablePrefix}.${ParquetTransactionsColumns.height}`
      : ParquetTransactionsColumns.height;
    const idCol = tablePrefix
      ? `${tablePrefix}.${ParquetTransactionsColumns.id}`
      : ParquetTransactionsColumns.id;
    return this
      .sql`ORDER BY ${this.sql.raw(heightCol)} ${this.sql.raw(sortOrder)}, ${this.sql.raw(idCol)}`;
  }
...
export class QueryValidator {
  ...
  static validateTransactionFilter(filter: TransactionsQueryFilter): void {
    if (
      ...
      (filter.first < 1 || filter.first > 1000)
    ) {
    ...
    if (
      filter.owners &&
      filter.owners.some(
        (owner) => !owner || (typeof owner === "string" && owner.length !== 43),
      )
    ) {
  ...
  static validateBlockFilter(filter: BlocksQueryFilter): void {
    ...
    if (
      filter.ids &&
      filter.ids.some(
        (id) => !id || (typeof id === "string" && id.length !== 64),
      )
    ) {
