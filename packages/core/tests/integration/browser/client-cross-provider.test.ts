...
describe("Browser Integration - Client Cross-Provider Validation", () => {
  ...
  let parquetProvider: ReturnType<typeof createWebParquetProvider>;
  ...
  beforeAll(async () => {
    ...
    // Create Parquet provider with HTTP-served parquet files (using DuckDB WASM)
    const datasetsBaseUrl = `${arIONode.apiUrl}/local/datasets`;
    parquetProvider = createWebParquetProvider({
      parquetUrls: {
  ...
  it("should validate parquet data against GraphQL endpoint in browser", async () => {
  ...
  it("should query same transaction from both clients in browser", async () => {
    console.log("\n🔄 Testing browser client with different providers...\n");
    ...
    // Get from parquet via client (DuckDB WASM)
    console.log("Querying via Parquet client (DuckDB WASM)...");
  ...
  it("should handle provider fallback scenarios in browser", async () => {
    console.log("\n🔀 Testing browser provider fallback scenarios...\n");
