...
export async function startArIONode(): Promise<ArIONodeContainer> {
  const fixturesPath = resolve(__dirname, "../../../../fixtures/parquet");
 ...
 */
export async function stopArIONode(arIONode: ArIONodeContainer): Promise<void> {
 ...
 */
export async function verifyArIONode(
  ...
  const parquetResponse = await fetch(transactionsParquetUrl, {
    method: "HEAD",
  });
