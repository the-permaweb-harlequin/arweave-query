...
describe("Node Integration - Parquet Query", () => {
  let arIONode: ArIONodeContainer;
  let provider: ReturnType<typeof createNodeParquetProvider>;
  ...
  beforeAll(async () => {
    ...
    provider = createNodeParquetProvider({
      ...
      duckdbConfig: {
        memory: ":memory:",
        readOnly: false, // Explicitly set to false for in-memory databases
      },
    });
  });
  ...
  it("should query transactions from parquet datasets", async () => {
  ...
  it("should filter transactions by owner", async () => {
    ...
    // Now query by that owner - using base64 string comparison
    const result = await provider.getTransactions({
      first: 10,
      owners: [ownerAddress],
    });
  ...
  it("should filter transactions by block height range", async () => {
  ...
  it("should query blocks from parquet datasets", async () => {
  ...
  it("should get a single transaction by id", async () => {
    ...
    // Now get that specific transaction - using base64 string comparison
